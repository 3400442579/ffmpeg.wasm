...
# 检查是否设置了 AUDIO_ONLY 环境变量
if [ "${AUDIO_ONLY:-false}" = "true" ]; then
  # 音频专用配置
  CONF_FLAGS=(
    -I. 
    -I./src/fftools 
    ...
    ${FFMPEG_MT:+ -sINITIAL_MEMORY=128MB}    # 减少内存使用（音频不需要太多内存）
    ${FFMPEG_MT:+ -sPTHREAD_POOL_SIZE=8}     # 减少线程数
    ...
    # ffmpeg source code (保持不变，因为基本工具仍需使用)
