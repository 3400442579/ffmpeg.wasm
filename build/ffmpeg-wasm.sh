...
CONF_FLAGS=(
  ...
  -sNODERAWFS=0                           # 禁用原始文件系统以确保兼容性
  -sASSERTIONS=1                           # 启用断言以便调试
  -sALLOW_TABLE_GROWTH=1                   # 允许表增长
  ...
  -sINVOKE_RUN=0                            # 不自动调用main函数
  -sEXIT_RUNTIME=0                          # 不退出运行时
  -sERROR_ON_UNDEFINED_SYMBOLS=0           # 忽略未定义符号
  ...
  ${FFMPEG_ST:+ -sINITIAL_MEMORY=64MB -sALLOW_MEMORY_GROWTH -sMAXIMUM_MEMORY=2048MB} # 增加初始内存，允许内存增长
